...
def read_secret_file(secret_name: str) -> str:
    ...
    logger.debug(f"🔍 Secret '{secret_name}' not found in any location")
    return None
...
def get_config_value(vault_path: str, vault_key: str, file_name: str = None, env_name: str = None, default_value: str = ""):
    ...
    if file_name:
        ...
        if file_value is not None and file_value != "vault_required":
            ...
            logger.debug(f"Secret file {file_name} requires Vault - continuing to Vault")
    ...
    if ((file_name and read_secret_file(file_name) == "vault_required") or 
        ...
        logger.error(f"🚨 CRITICAL: {file_name or env_name} requires Vault but Vault is unavailable!")
...
def get_sensitive_config_value(vault_path: str, vault_key: str, file_name: str = None, env_name: str = None, default_value: str = ""):
    ...
    if vault_path and vault_key:
        try:
            vault_value = get_vault_secret(vault_path, vault_key)
            if vault_value is not None:
                logger.debug(f"✅ Sensitive config '{vault_key}' retrieved from Vault")
                return vault_value
        except Exception as e:
            logger.debug(f"Vault lookup failed for {vault_path}/{vault_key}: {e}")
    
    # 2. Try secret files (Kubernetes + local development)
    if file_name:
        file_value = read_secret_file(file_name)
        if file_value is not None and file_value != "vault_required":
            logger.debug(f"✅ Sensitive config '{file_name}' retrieved from secret file")
            return file_value
        elif file_value == "vault_required":
            logger.warning(f"⚠️ Secret file {file_name} requires Vault but Vault unavailable - checking other sources")
    
    # 3. Try environment variable (skip if it's a security placeholder)
    if env_name:
        env_value = os.getenv(env_name)
        if env_value is not None and env_value != "vault_required":
            logger.warning(f"⚠️ Sensitive config '{env_name}' using environment variable (less secure)")
            return env_value
        elif env_value == "vault_required":
            logger.debug(f"Environment variable {env_name} requires Vault - skipping env fallback")
    ...
    if default_value:
        logger.warning(f"⚠️ Sensitive config using default value - NOT SECURE for production")
    return default_value
...
def get_file_first_config_value(file_name: str, env_name: str, default_value: str = ""):
    ...
    if file_value is not None:
        logger.debug(f"✅ Config '{file_name}' retrieved from secret file")
        return file_value
    
    # 2. Try environment variable
    env_value = os.getenv(env_name)
    if env_value is not None:
        logger.debug(f"✅ Config '{env_name}' retrieved from environment")
        return env_value
...
def validate_critical_config():
    ...
    if critical_failures:
        ...
        if not vault_status:
            logger.error("🚨 VAULT IS UNAVAILABLE - APPLICATION CANNOT START SECURELY")
            logger.error("🔧 RESOLUTION: Ensure Vault is accessible and AppRole credentials are valid")
            return False
